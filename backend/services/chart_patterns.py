...
def _flat_base(history, close):
    ...
    return matched, f"40-session flat base broke above resistance {resistance:.2f}"
...
def _vcp(history, close):
    ...
    return matched, f"Volatility contracted from {contractions[0] * 100:.1f}% to {contractions[2] * 100:.1f}% before breakout"
...
def _ascending_triangle(history, close):
    ...
    return matched, f"Rising lows broke ascending-triangle resistance {resistance:.2f}"
...
def _cup_with_handle(history, close):
    ...
    return matched, f"Cup-and-handle pivot {rim:.2f} cleared after a {cup_depth * 100:.1f}% cup"
...
def _bull_flag(history, close):
    ...
    return matched, f"Bull flag followed a {pole_gain * 100:.1f}% advance and cleared {flag_resistance:.2f}"
...
def _double_bottom(history, close):
    ...
    return matched, f"Double bottom cleared neckline {neckline:.2f} after two aligned lows"


def detect_bullish_patterns(prices):
    ...
    for code, detector in detectors:
        matched, reason = detector(history, close)
        if matched:
            matches.append({"code": code, "label": PATTERN_LABELS[code], "reason": reason})
    return matches
